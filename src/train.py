...
SAVE_DIR = '/content/drive/MyDrive/apd_project/runs'
os.makedirs(SAVE_DIR, exist_ok=True)

# Argument Parser
parser = argparse.ArgumentParser(description='PyTorch ResNet18 Training with GVR, SAM, or SGD on CIFAR-10/100')
...
parser.add_argument('--noise-ratio', default=0.2, type=float,
                    help='noise ratio for noisy CIFAR10/100 (default: 0.2)')
...
parser.add_argument('--optimizer', default='gvr', type=str, choices=['gvr', 'sam', 'sgd'],
                    help='optimizer to use (gvr, sam, sgd)')
parser.add_argument('--rho', default=0.2, type=float,
                    help='rho parameter for SAM optimizer (default: 0.2)')
parser.add_argument('--adaptive', action='store_true', default=False,
                    help='use adaptive SAM (default: False)')
...
def train_epoch(train_loader, model, criterion, optimizer, epoch, device, args, logger):
    ...
    for i, data in enumerate(train_loader):
        ...
        # For SGD/SAM, we will only use img1
        ...
        if args.optimizer == 'gvr':
            ...
            # SAM requires an initial forward-backward pass to compute gradients
            # before its first_step, then a closure for the second step.

            # Ensure model is in train mode (SAM handles BatchNorm internally for its steps)
            model.train()
            # First forward-backward pass to get initial gradients for SAM's first_step
            optimizer.zero_grad() # Clear any old gradients
            output_initial = model(img1)
            loss_initial = criterion(output_initial, target)
            loss_initial.backward() # Compute initial gradients at original weights

            # Define the closure that performs the second forward/backward pass at perturbed weights
            def sam_closure():
                # This closure is called by SAM's step method AFTER it has perturbed the weights.
                # So, we should zero grads, do forward, loss, and backward at the perturbed weights.
                optimizer.zero_grad() # Zero gradients for the *perturbed* model state
                output = model(img1)  # Use one view for SAM
                loss = criterion(output, target)
                loss.backward()
                return loss

            # Call SAM's step, which will use the initial gradients and then call closure for the second step.
            loss_val = optimizer.step(sam_closure).item()
            losses.update(loss_val, img1.size(0))
            loss1_meter.update(0.0, img1.size(0)) # Not applicable for SAM
            loss2_meter.update(0.0, img1.size(0)) # Not applicable for SAM
            gvr_penalty_meter.update(0.0, img1.size(0)) # Not applicable for SAM
...
def main():
    ...
    base_optimizer_class = optim.SGD # SGD will be the base for GVR and SAM
    ...
    if args.optimizer == 'gvr':
        ...
        optimizer = utils.SAM(model.parameters(), base_optimizer_class,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #}, is_best)
        #logger.write(f'Best Test Accuracy: {best_acc:.3f}\n')
        #logger.write(f"Epoch {epoch} finished in {(time.time() - start_time):.2f} seconds.\n")

    logger.write(f"\n--- Training Complete --- \n")
    #logger.write(f"Final Best Test Accuracy: {best_acc:.3f}\n")
